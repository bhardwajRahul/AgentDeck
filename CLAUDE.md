...
- **Repo instructions**: `AGENTS.md` is the entry file Codex/OpenCode/Antigravity discover by convention. It requires `CLAUDE.md` first, then targeted `DEVELOPMENT_LOG.md` lookup instead of loading the full log. The active log holds the current and previous month (`pnpm devlog:check` gates it in CI; `pnpm devlog:archive` slices older months into [`docs/devlog/`](docs/devlog/README.md)) — grep a specific month file when you need history, never load all archives at once.
