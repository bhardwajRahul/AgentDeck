...
- **Repo instructions**: `AGENTS.md` is the entry file Codex/OpenCode/Antigravity discover by convention. It requires `CLAUDE.md` first, then targeted `DEVELOPMENT_LOG.md` lookup instead of loading the full log. Older months are archived under [`docs/devlog/`](docs/devlog/README.md) — check which months the active log still holds (`grep -n '^## 2026-' DEVELOPMENT_LOG.md | tail -1`) before assuming a window, grep a specific month file when you need history, and never load all archives at once.
