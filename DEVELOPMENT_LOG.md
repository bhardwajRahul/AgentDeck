...
**미해결(이번 변경과 무관)**: `agentdeck daemon restart` 가 성공을 실패로 보고한다.
2회 모두 `restart FAILED — no daemon with PID <n> is answering` 을 찍었지만 실제로는
다른 pid 가 포트를 잡고 정상 기동했다(검증이 fork 한 pid 를 보는데 최종 listener 는
re-exec 된 다른 pid). 사용자를 불필요한 재시도로 유도하므로 후속 수정 대상.
