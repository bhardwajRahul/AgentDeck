...
**남은 것.** (1) 이 파일 맨 위 prepend 충돌은 그대로다 — 항목별 파일(`docs/devlog/2026-09-10-<slug>.md`)로 가면 사라지지만
`head DEVELOPMENT_LOG.md` 라는 모든 에이전트의 진입 습관과 docs/apme.md 의 날짜 인용을 함께 옮겨야 하므로 별건. (2) `.claude/skills/` 포인터가 gitignore 라 새 클론에
슬래시 스킬이 없음. (3) 규칙 파일에 새 불변식을 더할 때 굵은 첫 문장 = 규칙, 나머지 = 근거 구조 유지.
