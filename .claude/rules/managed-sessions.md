...
**Daemon-persisted order pins for observed sessions (#273)**: `agentdeck order set/clear/list` → `POST/GET /sessions/order` → `SessionOrderStore` (`bridge/src/session-order-store.ts`, `~/.agentdeck/session-order.json`, atomic tmp+rename, daemon-written like timeline.json). Keys are the **bare id** (`rawSessionId`) so both id forms address one pin; TTL 30 days from last roster sighting, cap 256 (least-recently-seen evicted). The daemon overlays pins in the sessions enricher **before fold+sort** onto observed rows **without their own weight only** — a managed session's launch-time `--weight` and a remote session's pushed weight always win, and weight 0 is a clear, not a pin. No wire change: the value rides `SessionInfo.weight`, so the Codex fold's weight-band key keeps distinct pins unfolded. The Swift daemon neither reads the file nor serves the route — a parity gap the CLI names explicitly; do not remove the managed `--weight` path until the real-user tab-to-deck scenario in #273 validates the replacement.
