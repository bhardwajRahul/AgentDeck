...
| `shared/src/usage-presentation.ts` | `pnpm generate-usage-presentation` | Generated C++/Swift/Kotlin heading and Luna selection predicate; TS shared by Stream Deck and Ulanzi, C++ used by IPS10. Provider attribution table binds subscription metadata to usage rows. `usage-presentation.test.ts` gates drift and exhaustion/reset/unknown cases. |
