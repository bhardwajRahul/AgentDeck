...
- **Fish motion**: `EinkFishSchool`은 힘 기반 boids 대신 넓은 순환 경로를 따라 두 무리가 유영한다. 개체별 앞뒤 간격과 수심 간격을 유지하고, 선회는 화면 평면 회전이 아니라 옆모습의 폭이 줄었다 반대 방향으로 향하는 yaw 투영으로 표현한다. 몸체는 유선형이고 꼬리는 연속 위상으로 움직이며, 원경의 물고기는 에이전트 뒤로 가려진다. STREAMING/HOVERING은 같은 경로의 속도만 부드럽게 바꾸며, 25ms simulation tick과 Double 위상은 100ms/400ms 화면 갱신 및 반복 순환에서 경로 연속성을 유지한다.
