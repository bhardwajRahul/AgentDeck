...
- **LCD habitat in e-ink layout**: 일반 LCD의 e-ink projection에는 Blender에서 제작한 정적인 배경을 캐시해 GPU에서 투명 크리처 비트맵과 별도로 합성한다. 공식 에이전트 실루엣·눈의 빈 공간·이름표·실제 사용량과 물고기는 런타임에서 그리며, 크리처에는 좌상단 조명의 명암과 안티앨리어싱을 적용한다. 흑백 변환은 배경 로드 때 한 번만 수행하며, 화면 회전 시 배경의 비율을 유지해 중앙 crop한다. 실제 EPD에는 기존 고대비 환경을 유지한다. 원본 및 재생성: `assets/terrarium/`.
