...
- **Fish motion**: `EinkFishSchool`은 50ms 고정 simulation tick과 동시점 이웃 좌표로 계산한다. 가까운 물고기의 반발력·회전량을 제한하고 초기 간격을 확보한다. 흑백 400ms / 컬러 100ms 화면 갱신에서도 같은 경로를 유지하며, HOVERING 전환은 기존 좌표에서 목적지만 바꾼다.
