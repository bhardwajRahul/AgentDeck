...
| Session order pinning (`--weight` sort override) | Limited | Yes (compatibility) | Observed sessions can now be pinned daemon-first: the Node daemon persists weight pins in `~/.agentdeck/session-order.json` (`agentdeck order`, `POST /sessions/order`) and overlays them on observed rows before fold+sort, riding the existing `SessionInfo.weight` wire field. The managed launcher's launch-time `--weight` remains the compatibility path. The Swift daemon does not read the pin file; real-user ordering validation is tracked in Discussion #278 and issue #273 |
