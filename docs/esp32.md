...
- A persistent usage rail, including in detail mode, shows Claude/Codex windows and received z.ai windows / Antigravity credits. MCP is labeled separately and credits are not percentages. Circular quota gauges show used percentages and reported reset times. Missing windows are hidden; the entire rail disappears when no quota or credits are known. Zero percent remains visible and stale Claude windows disappear without hiding Codex. Input/output tokens, tool calls, and estimated cost are labeled as the daemon snapshot, never as selected-session or project totals.
