...
- Three weighted project spaces (one in the portrait preview) per page group canonical creatures by exact reported project name. Project ordering and creature numbers follow identities; attention takes priority when choosing visible creatures. Additional spaces appear automatically every 12 seconds; tapping the page caption pauses/resumes rotation. Each space shows up to three creatures and their activities; additional peers rotate every 8 seconds. Up to two attention agents stay visible within each project; remaining slots rotate fairly and always fill when enough peers exist. Character numbers match their activity rows, tool icons describe received activity, and a branch badge shows reported active children. The latest attributable ring event appears below them in 20px text. Unknown project names remain separate. Grouping means co-presence, not a measured delegation relationship. Creatures retain state badges, dim idle states, and touch-to-inspect detail.
- A persistent usage rail, including in detail mode, shows Claude/Codex windows and received z.ai windows. MCP is labeled separately. Antigravity raw credits are never shown; a confirmed plan uses the existing compact `AGY Pro`-style header chip, independently of quota availability. Circular quota gauges show used percentages and reported reset times. Rings keep a consistent size and color meaning, with centered provider names, used-window labels and secondary reset text. The rail sizes to its content. Missing windows are hidden; the entire rail disappears when no percentage windows are known. Zero percent remains visible and stale Claude windows disappear without hiding Codex. Input/output tokens, tool calls, and estimated cost are labeled as the daemon snapshot, never as selected-session or project totals.
...
- A compact voice strip shows listening, sending, waiting, speaking, error and wake-ready states even with controls closed; inactive muted status is hidden. A separate voice/speaker drawer keeps the existing hold-to-talk, wake toggle, playback stop, and volume controls available. Hold-to-talk targets the selected eligible session; the existing wake-word path remains dedicated to OpenClaw.
