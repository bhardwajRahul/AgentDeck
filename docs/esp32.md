...
Dashboard labels are English: IBM Plex Sans KR Bold 20px headings and 28px wordmark/quota values, Regular 20px activity text and Regular 16px metadata, with Korean fallback for received content. User and agent text retain their original language; large detail counts use a compact 36px digit subset. The generated fonts are IPS10-only and the full Hangul face uses uncompressed 2bpp to bound flash and avoid decompression during drawing.
