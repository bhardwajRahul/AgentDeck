...
The serial host currently supplies at most ten detailed sessions, prioritizing attention when oversubscribed. The aquarium rotates this received set and labels it as a priority view with `10 of N agents`; it does not imply that all N session details are available. Widget and text pools stay bounded independently of that reported total. Empty activity, event, reset, token, cost and quota fields do not reserve decorative placeholder rows.
