...
4. **`DEVELOPMENT_LOG.md`** — searchable recent history: the current month and the previous one, enforced by `pnpm devlog:check` in CI and restored by `pnpm devlog:archive` (`scripts/devlog-archive.mjs`, which slices older months into `docs/devlog/YYYY-MM.md` and regenerates the index). Never read it in full; check the top, then `rg` for keywords/filenames, and grep one archived month at a time.
