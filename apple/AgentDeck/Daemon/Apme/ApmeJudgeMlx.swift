...
enum ApmeJudgeMlx {
    ...
    /// budget, 90s-ceiling, or repetition-penalty measurement (measured on
    /// judge prompts, not the classifier's one-word answer).
    static func judge(prompt: String, config: ApmeJudgeConfig) async -> String? {
        ...
        let penalty = Self.resolvedPenalty(configured: config.repetitionPenalty, suppressed: Self.penaltySuppressed(for: url))
